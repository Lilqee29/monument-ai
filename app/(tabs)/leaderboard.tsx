/**
 * Leaderboard Tab — Inline rankings (no back button, tab-level).
 * Real Supabase data: XP from sessions, nations, sites.
 */
...
import { Zap, Globe, Trophy } from 'lucide-react-native';
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        {/* Your Rank Banner */}
        {myEntry && (
          <Animated.View entering={FadeInDown.delay(100)} style={styles.myBanner}>
            <Text style={styles.myBannerLabel}>Your Rank</Text>
            <Text style={styles.myBannerRank}>#{myRank}</Text>
            <Text style={styles.myBannerXP}>{myEntry.xp.toLocaleString()} XP</Text>
          </Animated.View>
        )}
...
function PodiumCard({ entry, rank, isMe, featured }: { entry: LeaderboardEntry; rank: number; isMe: boolean; featured?: boolean }) {
  return (
    <Animated.View entering={FadeInDown.delay(rank * 150)} style={[styles.podiumCard, featured && styles.podiumFeatured]}>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: '#c9a84c', fontSize: 22, fontFamily: 'Georgia' },
  headerSub: { color: '#9a9483', fontSize: 10, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1.5 },
  ...
  myBanner: {
    ...
    borderWidth: 1, borderColor: 'rgba(201,168,76,0.2)', padding: 16,
